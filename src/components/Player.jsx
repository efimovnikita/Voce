...
const Player = ({ isPlaying, onPlayPause, onRewind, playbackRate, onSpeedChange }) => {
  return (
    <div className="flex flex-col items-center w-full space-y-12">
      ...
      <div className="flex items-center justify-center w-full h-24 space-x-1.5">
        ...
        {[...Array(16)].map((_, i) => {
          ...
          return (
            <div 
              key={i}
              className={`w-2 rounded-full transition-all duration-500 ${!isPlaying ? 'bg-slate-800 h-2 opacity-50' : 'bg-blue-500 opacity-100'}`}
              style={{ 
      ...
      <div className="flex items-center justify-center space-x-10 mt-4">
        ...
        <button
          ...
          className="flex items-center justify-center w-14 h-14 text-slate-500 hover:text-slate-300 transition-colors focus:outline-none"
        >
        ...
        >
          {isPlaying ? (
            ...
            <svg className="w-10 h-10 ml-1.5" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" />
            </svg>
          )}
        </button>

        {/* Кнопка Скорости (Справа) */}
        <button
          onClick={onSpeedChange}
          aria-label="Change playback speed"
          className="flex items-center justify-center w-14 h-14 text-slate-500 hover:text-slate-300 transition-colors focus:outline-none font-bold text-lg select-none"
        >
          {playbackRate}x
        </button>
